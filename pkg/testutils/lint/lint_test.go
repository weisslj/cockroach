...
const cockroachDB = "github.com/weisslj/cockroach"
...
func TestLint(t *testing.T) {
	...
	t.Run("TestUnused", func(t *testing.T) {
		...
		linter := lint.Linter{
			...
			Ignores: []lint.Ignore{
				...
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/sql/parser/sql.go", Checks: []string{"U1000"}},
				// Generated file containing many unused postgres error codes.
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/sql/pgwire/pgerror/codes.go", Checks: []string{"U1000"}},
				...
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/sql/opt/optgen/exprgen/custom_funcs.go", Checks: []string{"U1000"}},
			},
		}
	...
	t.Run("TestForbiddenImports", func(t *testing.T) {
		...
		forbiddenImports := map[string]string{
			...
			"golang.org/x/sync/singleflight":   "github.com/weisslj/cockroach/pkg/util/syncutil/singleflight",
			"syscall":                          "sysutil",
		}
		...
		settingsPkgPrefix := `github.com/weisslj/cockroach/pkg/settings`
		if err := stream.ForEach(stream.Sequence(
			filter,
			stream.Sort(),
			stream.Uniq(),
	...
	t.Run("TestMegacheck", func(t *testing.T) {
		...
		for checker, ignores := range map[lint.Checker][]lint.Ignore{
			...
			&hashChecker{}:  nil,
			simple.NewChecker(): {
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/security/securitytest/embedded.go", Checks: []string{"S1013"}},
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/ui/embedded.go", Checks: []string{"S1013"}},
			},
			staticcheck.NewChecker(): {
				...
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/sql/parser/sql.go", Checks: []string{"SA4006"}},
				...
				&lint.GlobIgnore{Pattern: "github.com/weisslj/cockroach/pkg/*/*/*.pb.gw.go", Checks: []string{"SA1019"}},
			},
		} {
