...
import (
	...
	"github.com/weisslj/cockroach/pkg/sql/distsqlplan"
	"github.com/weisslj/cockroach/pkg/sql/distsqlplan/replicaoracle"
...
func (dsp *DistSQLPlanner) addAggregators(
	...
	if !multiStage {
		...
		for i, o := range n.groupColOrdering {
			...
			if !found {
				return pgerror.NewAssertionErrorf("group column ordering contains non-grouping column %d", o.ColIdx)
			}
			if o.Direction == encoding.Descending {
